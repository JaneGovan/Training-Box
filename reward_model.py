...
class RMBox:
    ...
    def concat_function(self, examples):
        prompt_chosen = examples["prompt"]+examples["chosen"]
        prompt_rejected = examples["prompt"]+examples["rejected"]
        tokenized_chosen = self.tokenizer(prompt_chosen)
        tokenized_rejected = self.tokenizer(prompt_rejected)
        return {
...
if __name__ == "__main__":
    parser = HfArgumentParser((DataArguments, ModelArguments, RewardConfig, ScriptArguments))
    parser.add_argument("-f","--yaml-file", type=str, default="./yaml/reward_model.yaml", help="The YAML configuration file for training")
    ...
    rm_b = RMBox(data_args, model_args, train_args, lora_args)
    rm_b.train()