...
class SFTBox:
    ...
    def preprocess_func(self, examples):
        ...
        for idx, q in enumerate(examples["instruction"]):
            prompt = q + examples["input"][idx]
            response = examples["output"][idx]
            ...
            labels = [LabelSmoother.ignore_index]*(len(source_ids)) + target_ids + [self.tokenizer.eos_token_id]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f","--yaml-file", type=str, default="./yaml/sft.yaml", help="The YAML configuration file for training")
    ...
    sft_b = SFTBox(data_args, model_args, train_args, lora_args)
    sft_b.train()