...
@dataclass
class TrainArguments(GRPOConfig):
...
class GRPOBox:
    ...
    def preprocess_func(self, examples):
        new_samples = {"prompt": [], "answer": examples["output"]}
        for ins, query, ans in zip(examples["instruction"], examples["input"], examples["output"]):
            prompt = ins+query
            new_samples["prompt"].append(
                [
    ...
    def reward_function(self, completions, **kwargs):
        rewards = []
        ...
        for completion in completion_contents:
            ...
            if 'mount elbert' in text:
                correctness_reward += 0.6
            if 'denali' not in text and 'mount mckinley' not in text:
                correctness_reward += 0.4
            # 注意：这里正确性奖励最高为1.0

            # 多样性奖励
            unique_chars_ratio = len(set(completion)) / len(completion) if completion else 0
            diversity_reward = unique_chars_ratio * 0.05  # 0-0.05
            ...
            total_reward = correctness_reward + diversity_reward + length_reward
            rewards.append(total_reward)

        return rewards

    def reward_function2(self, completions, answer, **kwargs):
        rewards = []
        completion_contents = [completion[0]["content"] for completion in completions]
        for completion, ans in zip(completion_contents, answer):
            rewards.append(calculate_bleu(ans, completion))
        return rewards
    ...
    def train(self):
        ...
        trainer = GRPOTrainer(
            model=model,
            reward_funcs=[self.reward_function2],
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f","--yaml-file", type=str, default="./yaml/grpo.yaml", help="The YAML configuration file for training")
    ...
    grpo_b = GRPOBox(data_args, model_args, train_args, lora_args)
    grpo_b.train()
