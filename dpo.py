...
class DPOBox:
    ...
    def preprocess_data(self):
        ...
        return raw_datasets.shuffle(seed=self.train_args.seed)
    ...
    def train(self):
        ...
        trainer = DPOTrainer(
            model=model,
            ref_model= None if self.lora_args.use_peft else copy.deepcopy(model),
...
if __name__ == "__main__":
    parser = HfArgumentParser((DataArguments, ModelArguments, DPOConfig, ScriptArguments))
    parser.add_argument("-f","--yaml-file", type=str, default="./yaml/dpo.yaml", help="The YAML configuration file for training")
    ...
    dpo_b = DPOBox(data_args, model_args, train_args, lora_args)
    dpo_b.train()